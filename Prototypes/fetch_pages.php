...
if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $results = $mysqli->prepare("SELECT id, name, artist FROM crud ORDER BY id ASC LIMIT $page_position, $item_per_page");
    $results->execute(); //Execute prepared Query
    $results->bind_result($id, $name, $artist); //bind variables to prepared statement

    //Display records fetched from database.
    echo '<ul class="contents">';
    while ($results->fetch()) { //fetch values
        echo '<li>';
        echo  $id. '. <strong>' .$name.'</strong> &mdash; '.$artist;
        echo '</li>';
    }
    echo '</ul>';
